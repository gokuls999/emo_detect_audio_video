...
RESUME_PATH    = CHECKPOINT_DIR / "stage5_resume.pt"
FINAL_PATH     = CHECKPOINT_DIR / "stage5_final.pt"
...
def run(config_path: str) -> None:
    ...
    loader = build_dataloader(ds_cfg, "train", batch_size=cfg["training"]["batch_size"], num_workers=cfg["training"]["num_workers"], augment=True)

    model = MSTDNAStudent().to(device)
    prior = CHECKPOINT_DIR / "stage4_caption_refined.pt"
    if prior.exists():
        model.load_state_dict(torch.load(prior, map_location=device))
        print(f"Loaded student from {prior}")
    else:
        print("WARNING: stage4 checkpoint not found — student is random init")
    ...
    if RESUME_PATH.exists():
        ...
        if all(torch.isfinite(p).all() for p in ckpt["model"].values()):
            ...
            print(f"Resuming stage5 from epoch {start_epoch}/{total_epochs}")
        else:
            print("Stage5 checkpoint has NaN — starting fresh.")
    else:
        print("Starting stage5 fresh.")

    for epoch in range(start_epoch, total_epochs):
        model.train()
        totals: dict[str, float] = {}
        skipped = 0
        for batch in tqdm(loader, desc=f"stage5 epoch {epoch + 1}"):
            optimizer.zero_grad()
            ...
            loss    = multi_task_loss(outputs, batch_t)["loss"]
    ...
    print(f"Stage 5 complete. Saved to {FINAL_PATH}")


if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--config", default="configs/stage5_augment.yaml")
